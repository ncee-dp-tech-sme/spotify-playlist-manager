 ...
 */
async function loadAndRenderDetails(container: HTMLElement, playlistId: string): Promise<void> {
  ...
  const toggleDupes = container.querySelector<HTMLInputElement>('#toggle-dupes')!;
  toggleDupes.checked = false;
  toggleDupes.onchange = () => {
    renderRows(tbody, items, duplicateIds, toggleDupes.checked);
  };

  const removeDupesBtn = container.querySelector<HTMLButtonElement>('#btn-remove-dupes')!;
  if (redundantDuplicatesCount > 0) {
    removeDupesBtn.style.display = 'inline-flex';
    removeDupesBtn.onclick = async () => {
      const confirmRemove = window.confirm(
        `Are you sure you want to remove ${redundantDuplicatesCount} duplicate track occurrence(s) from this playlist? Unique tracks will be kept in place.`,
      );
      if (!confirmRemove) return;

      removeDupesBtn.disabled = true;
      const statusEl = main.querySelector<HTMLElement>('#detail-status')!;
...
function buildTrackRow(item: PlaylistItem, index: number, duplicateIds: Set<string>): string {
