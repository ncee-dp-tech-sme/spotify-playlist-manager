...
import { addTracksToPlaylist, fetchAllPlaylistItems, fetchAllUserPlaylists, getCurrentUserProfile, } from '../api/playlists.js';
...
function renderSelectors(main, playlists) {
  ...
  `;
    main.querySelector('#btn-compare-go').addEventListener('click', async () => {
        const selA = (main.querySelector('#sel-a')).value;
        const selB = (main.querySelector('#sel-b')).value;
        if (!selA || !selB) {
...
async function runComparison(main, plA, plB) {
    const resultEl = main.querySelector('#compare-result');
    ...
    const idsA = new Set(itemsA.map((i) => i.item?.id).filter(Boolean));
    const idsB = new Set(itemsB.map((i) => i.item?.id).filter(Boolean));
 ...
 */
function renderCompareResult(container, plA, plB, itemsA, itemsB, idsA, idsB) {
  ...
  `;
    ...
    const hideBothCheckbox = container.querySelector('#toggle-hide-both');
    ...
    container.querySelector('#btn-transfer-a-to-b').addEventListener('click', () => handleTransfer(container, plB.id, 'a', plA, plB, idsA, idsB));
    container.querySelector('#btn-transfer-b-to-a').addEventListener('click', () => handleTransfer(container, plA.id, 'b', plA, plB, idsA, idsB));
}
/**
 * Toggle visibility of rows containing tracks present in both playlists.
 * Fri Sep  4 01:03:31 CEST 2026
 */
function updateCompareVisibility(container, hideInBoth) {
    const bothRows = container.querySelectorAll('.row-both');
 ...
 * Fri Sep  4 01:03:31 CEST 2026
 */
function setColumnCheckboxes(container, side, checked) {
    const checkboxes = container.querySelectorAll(`.track-checkbox[data-side="${side}"]`);
 ...
 * Fri Sep  4 01:03:31 CEST 2026
 */
function buildCompareTable(items, otherIds, side, hideInBoth = false) {
    if (items.length === 0)
        return '<p class="empty">No tracks.</p>';
    const rows = items
        .map((item, idx) => {
...
async function handleTransfer(container, toPlaylistId, sourceSide, plA, plB, idsA, idsB) {
    const checkboxes = container.querySelectorAll(`.track-checkbox[data-side="${sourceSide}"]:checked`);
    const uris = [...checkboxes].map((cb) => cb.dataset['uri']).filter(Boolean);
    if (uris.length === 0) {
        showTransferStatus(container, 'No tracks selected.', 'warn');
        return;
    }
    const statusEl = container.querySelector('#transfer-status');
...
async function refreshColumn(container, playlist, otherIds, side) {
    const col = container.querySelector(`#col-${side}`);
    const tableWrapper = col.querySelector('.table-wrapper');
    if (tableWrapper)
        tableWrapper.innerHTML = '<div class="loading">Refreshing…</div>';
    try {
...
function showNotice(main, message, type) {
...
function showTransferStatus(container, message, type) {
    const el = container.querySelector('#transfer-status');
