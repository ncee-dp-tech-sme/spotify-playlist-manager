...
import type { SpotifyApiError } from './types.js';
 ...
 */
export async function spotifyDelete<T>(path: string, body?: unknown): Promise<T> {
  return spotifyRequest<T>('DELETE', path, body);
}
...
async function spotifyRequest<T>(
  ...
  if (response.status === 429) {
    ...
    await sleep(delay);
    return spotifyRequest<T>(method, path, body, retryCount + 1);
  }
  ...
  if (!response.ok) {
    ...
    try {
      const json = (await response.json()) as { error: SpotifyApiError };
