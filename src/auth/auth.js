...
import { clearTokens, consumeCodeVerifier, isTokenExpired, loadTokens, saveCodeVerifier, saveTokens, } from './storage.js';
...
export function validateEnv() {
    ...
    if (!REDIRECT_URI)
        missing.push('VITE_SPOTIFY_REDIRECT_URI');
    if (!SCOPES)
        missing.push('VITE_SPOTIFY_SCOPES');
    if (missing.length) {
...
export async function handleCallback() {
    ...
    if (!code)
        throw new Error('No authorization code returned by Spotify.');
    if (!state || state !== storedState) {
        throw new Error('OAuth state mismatch — possible CSRF attempt. Please try again.');
    }
    const verifier = consumeCodeVerifier();
    if (!verifier)
        throw new Error('Missing PKCE code_verifier. Please start the login flow again.');
...
function parseTokenResponse(json, fallbackRefreshToken) {
